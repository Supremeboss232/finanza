...
async def create_investment_for_current_user(
    ...
    return await create_user_investment(db=db_session, investment=investment, user_id=current_user.id)
...
async def read_investments_for_current_user(
    ...
    return await get_user_investments(db=db_session, user_id=current_user.id, skip=skip, limit=limit)
