...
async def create_loan_for_current_user(
    ...
    return await create_user_loan(db=db_session, loan=loan, user_id=current_user.id)
...
async def read_loans_for_current_user(
    ...
    return await get_user_loans(db=db_session, user_id=current_user.id, skip=skip, limit=limit)
