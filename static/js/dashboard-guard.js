...
(function() {
    ...
    async function verifyTokenOnBackend(token) {
        try {
            ...
            console.log('[GUARD] Token verification response:', response.status);
            ...
            // 401 = Token invalid/expired - should redirect to signin
            if (response.status === 401) {
                console.warn('[GUARD] Backend rejected token (401)');
                return false;
            }
            
            // 500 or other errors - allow page to load anyway (user can retry)
            // This prevents users from getting locked out due to server issues
            if (response.status >= 500) {
                console.warn('[GUARD] Server error during token verification (status ' + response.status + ') - allowing page load');
                return true; // Allow page to load, let data fetches handle auth
            }
            
            // Other status codes - allow page to load
            console.warn('[GUARD] Unexpected status during token verification: ' + response.status + ' - allowing page load');
            return true;
            
        } catch (e) {
            console.error('[GUARD] Token verification error (network/timeout):', e.message);
            // Network errors might be temporary - allow page to load
            // Data fetch will handle authentication if needed
            return true;
        }
    }
