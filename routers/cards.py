...
async def create_card_for_current_user(
    ...
    return await create_user_card(db=db_session, card=card, user_id=current_user.id)
...
async def read_cards_for_current_user(
    ...
    return await get_user_cards(db=db_session, user_id=current_user.id, skip=skip, limit=limit)
